...
- `src/pages/admin-analytics.tsx` - Analytics tab: revenue charts, order stats, page view tracking (daily chart, session stats, device/browser/OS breakdown, new vs returning, bounce rate, avg duration, pages/session), visitor geolocation (cities, countries), cart abandonment funnel, SC metrics, test stats, engagement, voucher usage, conventions, IP exclusion management
