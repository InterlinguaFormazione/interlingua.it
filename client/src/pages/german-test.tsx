...
const CEFR_DESCRIPTIONS: Record<string, { label: string; desc: string }> = {
  A1: { label: "A1 - Anfänger", desc: "Ich kann sehr einfache Sätze verstehen und mich in einfachen Situationen vorstellen." },
  A2: { label: "A2 - Grundkenntnisse", desc: "Ich kann einfache Alltagsgespräche führen und kurze Nachrichten schreiben." },
  B1: { label: "B1 - Mittelstufe", desc: "Ich kann über vertraute Themen sprechen, Hauptinhalte klarer Texte verstehen und einfache zusammenhängende Texte schreiben." },
  B2: { label: "B2 - Obere Mittelstufe", desc: "Ich kann an detaillierten Diskussionen teilnehmen, komplexe Texte verstehen und klare Aufsätze schreiben." },
  C1: { label: "C1 - Fortgeschritten", desc: "Ich kann mich fließend und spontan ausdrücken, anspruchsvolle Texte verstehen und gut strukturierte, detaillierte Texte verfassen." },
};

const SKILL_LABELS: Record<string, string> = {
  grammar: "Grammatik",
  vocabulary: "Wortschatz",
  use_of_english: "Sprachgebrauch",
...
export default function GermanTestPage() {
  ...
  const playTestAudio = useCallback(() => {
    ...
    if (!testAudioRef.current) {
      const audio = new Audio("/audio/german/listening_A0_000.mp3");
  ...
  const handleSelfAssessment = async (level: string) => {
    ...
    try {
      const res = await apiRequest("POST", "/api/german-test/start", {
        ...registrationData,
  ...
  const handleAnswer = async () => {
    ...
    try {
      const res = await apiRequest("POST", "/api/german-test/answer", {
        sessionId,
  ...
  const handleWritingSubmit = async () => {
    ...
    setIsSubmitting(true);
    try {
      const res = await apiRequest("POST", "/api/german-test/submit-writing", {
        sessionId,
      ...
      if (data.success) {
        ...
        const skipRes = await apiRequest("POST", "/api/german-test/complete-without-speaking", { sessionId });
        ...
        {(phase === "registration" || phase === "audio-check" || phase === "self-assessment") && (
          <div className="text-center mb-10">
            ...
            <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-slate-900 via-blue-900 to-indigo-900 dark:from-white dark:via-blue-100 dark:to-indigo-200 bg-clip-text text-transparent leading-normal pb-2" data-testid="text-page-title">
              Entdecke Dein<br className="hidden md:block" /> Deutschniveau
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-3 text-base max-w-lg mx-auto">
              Ein kostenloser und intelligenter Test, der sich an deine Antworten anpasst, um deine Sprachkenntnisse präzise zu bewerten.
            </p>
          </div>
        )}
        {phase !== "registration" && phase !== "audio-check" && phase !== "self-assessment" && (
            ...
            {phase === "mc-questions" && (
              <div className="mt-3">
                ...
                <div className="flex justify-between mt-1 text-xs text-slate-500">
                  {["Gramm", "Wortsch", "Sprach", "Lesen", "Hören"].map((label, idx) => (
          ...
          <div className="max-w-xl mx-auto" data-testid="card-audio-check">
            <div className="rounded-3xl bg-white/80 dark:bg-slate-800/70 backdrop-blur-xl border border-white/90 dark:border-slate-700/60 shadow-2xl shadow-blue-900/5 dark:shadow-black/20 overflow-hidden">
              <div className="px-8 pt-8 pb-4 text-center">
                ...
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">Audio-Überprüfung</h2>
                <p className="text-slate-400 dark:text-slate-500 text-sm mt-2">
                  Bevor wir beginnen, überprüfen wir, ob Ihr Audio richtig funktioniert.
                </p>
                <p className="text-amber-600 dark:text-amber-400 text-xs mt-4 max-w-sm mx-auto">
                  Für eine genauere Bewertung empfehlen wir, den Test mit aktiviertem Audio durchzuführen.
                </p>
                <button
                  onClick={() => {
                    ...
                    setPhase("self-assessment");
                    toast({ title: "Audio deaktiviert", description: "Die Hörverstehen-Fragen werden als Text angezeigt.", variant: "destructive" });
                  }}
                ...
                >
                  <VolumeX className="w-4 h-4" /> Ich habe kein Audio — überspringen
                </button>
              </div>
                ...
                <div className="space-y-5" data-testid="audio-check-listening">
                  <div className="rounded-2xl bg-indigo-50/80 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800 p-5">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-sm font-medium text-indigo-700 dark:text-indigo-300">Audio-Test</span>
                    </div>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
                      Klicken Sie auf die Schaltfläche, um eine Test-Audiodatei abzuspielen. Stellen Sie sicher, dass Sie sie deutlich hören.
                    </p>
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {testAudioPlaying ? (
                          <><Volume2 className="w-4 h-4 animate-pulse" /> Wird abgespielt...</>
                        ) : (
                          <><Play className="w-4 h-4" /> Audio abspielen</>
                        )}
                      </button>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    ...
                    >
                      <CheckCircle className="w-4 h-4 mr-2" /> Ich höre den Ton
                    </Button>
                    <Button
                      onClick={() => {
                        setAudioAvailable(false);
                        confirmListeningOk();
                        toast({ title: "Achtung", description: "Die Hörverstehen-Fragen werden als Text angezeigt.", variant: "destructive" });
                      }}
                      variant="outline"
                    ...
                    >
                      <Volume2 className="w-4 h-4 mr-2" /> Ich habe kein Audio
                    </Button>
                  </div>
                </div>
              </div>
          ...
          <div className="max-w-xl mx-auto" data-testid="card-self-assessment">
            <div className="rounded-3xl bg-white/80 dark:bg-slate-800/70 backdrop-blur-xl border border-white/90 dark:border-slate-700/60 shadow-2xl shadow-blue-900/5 dark:shadow-black/20 overflow-hidden">
              <div className="px-8 pt-8 pb-4 text-center">
                ...
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">Come valuti il tuo tedesco?</h2>
                <p className="text-slate-400 dark:text-slate-500 text-sm mt-2">
                ...
                <CardContent className="space-y-6">
                  {currentQuestion.skillType === "listening" && currentQuestion.audioUrl && audioAvailable && (
                    <div className="rounded-xl p-4 border bg-indigo-50 dark:bg-indigo-950/30 border-indigo-200 dark:border-indigo-800 space-y-3" data-testid="audio-player">
                      ...
                      <div className="text-xs text-slate-500">Wiedergaben übrig: {listeningPlaysLeft}</div>
                      {listeningPlaysLeft <= 0 && !isAudioPlaying && (
                        <p className="text-xs text-amber-600 dark:text-amber-400">Sie haben alle Wiedergaben aufgebraucht. Bitte beantworten Sie die Frage.</p>
                      )}
                    </div>
                  )}
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        {currentQuestion.skillType === "listening" ? (
                          <>
                            ...
                            <span className="text-xs uppercase tracking-wider text-indigo-500 font-medium">Transkript (Lesemodus)</span>
                          </>
                        ) : (
          ...
          <div className="space-y-6" data-testid="section-results">
            <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-10 text-center text-white">
                <p className="text-blue-100 text-sm mb-3 uppercase tracking-wider">Your German Level</p>
