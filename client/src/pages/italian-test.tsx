...
export default function ItalianTestPage() {
  ...
  const playTestAudio = useCallback(() => {
    ...
    if (!testAudioRef.current) {
      const audio = new Audio("/audio/italian/listening_A0_000.mp3");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50/50 to-violet-50 dark:from-slate-900 dark:via-blue-950 dark:to-indigo-950 relative overflow-hidden notranslate" translate="no">
      <Breadcrumb items={[{ label: "Test di Livello", href: "/test-di-livello" }, { label: "Test di Italiano" }]} schemaOnly />
          ...
          <div className="max-w-xl mx-auto" data-testid="card-audio-check">
            <div className="rounded-3xl bg-white/80 dark:bg-slate-800/70 backdrop-blur-xl border border-white/90 dark:border-slate-700/60 shadow-2xl shadow-blue-900/5 dark:shadow-black/20 overflow-hidden">
              <div className="px-8 pt-8 pb-4 text-center">
                ...
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">Controllo Audio</h2>
                ...
                <CardContent className="space-y-6">
                  {currentQuestion.skillType === "listening" && currentQuestion.audioUrl && audioAvailable && (
                    <div className="rounded-xl p-4 border bg-indigo-50 dark:bg-indigo-950/30 border-indigo-200 dark:border-indigo-800 space-y-3" data-testid="audio-player">
                      ...
                      <div className="text-xs text-slate-500">Ascolti rimasti: {listeningPlaysLeft}</div>
                      {listeningPlaysLeft <= 0 && !isAudioPlaying && (
                        <p className="text-xs text-amber-600 dark:text-amber-400">Hai esaurito gli ascolti. Rispondi alla domanda.</p>
                      )}
                    </div>
                  )}
