...
import { Briefcase, CheckCircle, ChevronRight, Loader2, Mic, MicOff, PenTool, Volume2, BookOpen, Brain, MessageSquare, Shield, Clock, ArrowRight, User, Mail, Phone, Building2, MapPin, Map } from "lucide-react";
...
const registrationSchema = z.object({
  ...
  company: z.string().min(1, "Campo obbligatorio"),
...
const CEFR_DESCRIPTIONS: Record<string, { label: string; desc: string }> = {
  ...
  B1: { label: "B1 - Intermediate", desc: "I can talk about familiar topics and write simple connected texts." },
  B2: { label: "B2 - Upper Intermediate", desc: "I can engage in detailed discussions and write clear essays." },
  C1: { label: "C1 - Advanced", desc: "I can express myself fluently and produce well-structured writing." },
};
...
const API_BASE = "/api/business-english-test";
...
export default function BusinessEnglishTestPage() {
  ...
  const handleSelfAssessment = async (level: string) => {
    ...
    try {
      const res = await apiRequest("POST", `${API_BASE}/start`, {
        ...registrationData,
  ...
  const handleAnswer = async () => {
    ...
    try {
      const res = await apiRequest("POST", `${API_BASE}/answer`, {
        sessionId,
  ...
  const handleWritingSubmit = async () => {
    ...
    setIsSubmitting(true);
    try {
      const res = await apiRequest("POST", `${API_BASE}/submit-writing`, {
        sessionId,
  ...
  const handleSpeakingSubmit = async () => {
    ...
    try {
      if (!speakingSubmitted) {
        ...
        const res = await fetch(`${API_BASE}/submit-speaking`, {
          method: "POST",
          body: formData,
        });
        const data = await res.json();
  ...
  const handleSkipSpeaking = async () => {
    ...
    setIsSubmitting(true);
    try {
      const res = await apiRequest("POST", `${API_BASE}/complete-without-speaking`, { sessionId });
      const data = await res.json();
      if (data.success) {
        setResults({
  ...
  const completeTest = async () => {
    if (!sessionId) return;
    try {
      const res = await apiRequest("POST", `${API_BASE}/complete`, { sessionId });
      const data = await res.json();
      if (data.success) {
        setResults({
          finalLevel: data.finalLevel,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50/30 to-teal-50 dark:from-slate-900 dark:via-emerald-950/30 dark:to-teal-950 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-40 -right-40 w-96 h-96 bg-emerald-200/30 dark:bg-emerald-800/10 rounded-full blur-3xl" />
        <div className="absolute top-1/3 -left-32 w-80 h-80 bg-teal-200/25 dark:bg-teal-800/10 rounded-full blur-3xl" />
        <div className="absolute -bottom-20 right-1/4 w-72 h-72 bg-cyan-200/20 dark:bg-cyan-800/10 rounded-full blur-3xl" />
      </div>
      <div className="relative container mx-auto px-4 py-10 max-w-4xl">
        {(phase === "registration" || phase === "self-assessment") && (
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm text-emerald-700 dark:text-emerald-300 px-5 py-2 rounded-full text-sm font-medium mb-5 border border-emerald-100 dark:border-emerald-800/40 shadow-sm">
              <Briefcase className="w-4 h-4" />
              Business English Assessment
            </div>
            <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-slate-900 via-emerald-900 to-teal-900 dark:from-white dark:via-emerald-100 dark:to-teal-200 bg-clip-text text-transparent leading-tight" data-testid="text-page-title">
              Business English<br className="hidden md:block" /> Proficiency Test
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-3 text-base max-w-lg mx-auto">
              A quick, adaptive assessment to evaluate your professional English skills. 5 questions per section for a faster, focused evaluation.
            </p>
          </div>
        )}
        {phase !== "registration" && phase !== "self-assessment" && (
              ...
              <div className="mt-3">
                  ...
                  {["grammar", "vocabulary", "use_of_english", "reading", "listening"].map((skill, idx) => (
                    <div
                      ...
                      className={`flex-1 h-2 rounded-full ${
                        idx + 1 < currentSectionIndex ? "bg-emerald-500" :
                        idx + 1 === currentSectionIndex ? "bg-teal-500" :
                ...
                <div className="flex justify-between mt-1 text-xs text-slate-500">
                  {["Grammar", "Vocab", "Use of Eng", "Reading", "Listening"].map((label, idx) => (
                    <span key={idx} className={idx + 1 === currentSectionIndex ? "text-teal-600 font-medium" : ""}>
                      {label}
                    </span>
                  ))}
                </div>
          ...
          <div className="max-w-xl mx-auto" data-testid="card-registration">
            <div className="grid grid-cols-3 gap-4 mb-8 max-w-sm mx-auto">
              <div className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-white/50 dark:bg-slate-800/40 backdrop-blur-sm border border-white/80 dark:border-slate-700/50">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400 text-center">Adaptive</span>
              </div>
              <div className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-white/50 dark:bg-slate-800/40 backdrop-blur-sm border border-white/80 dark:border-slate-700/50">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400 text-center">Business</span>
              </div>
              <div className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-white/50 dark:bg-slate-800/40 backdrop-blur-sm border border-white/80 dark:border-slate-700/50">
                <div className="w-10 h-10 rounded-xl bg-cyan-100 dark:bg-cyan-900/40 flex items-center justify-center">
                  <Clock className="w-5 h-5 text-cyan-600 dark:text-cyan-400" />
                </div>
                <span className="text-xs text-slate-500 dark:text-slate-400 text-center">~15 min</span>
              </div>
            </div>

            <div className="rounded-3xl bg-white/80 dark:bg-slate-800/70 backdrop-blur-xl border border-white/90 dark:border-slate-700/60 shadow-2xl shadow-emerald-900/5 dark:shadow-black/20 overflow-hidden">
              <div className="px-8 pt-8 pb-2">
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">Company & Candidate Details</h2>
                <p className="text-slate-400 dark:text-slate-500 text-sm mt-1">Fields marked with * are required</p>
              </div>
                  ...
                  <form onSubmit={form.handleSubmit(handleRegistration)} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <FormField control={form.control} name="firstName" render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">First Name *</FormLabel>
                          <FormControl>
                            <div className="relative group">
                              <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 dark:text-slate-600 group-focus-within:text-emerald-500 transition-colors" />
                              <Input {...field} data-testid="input-first-name" placeholder="First name" className="pl-11 h-12 bg-slate-50/80 dark:bg-slate-900/40 border-slate-200/80 dark:border-slate-600/60 focus:border-emerald-400 dark:focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 rounded-xl transition-all duration-200 focus:ring-4 focus:ring-emerald-500/10" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      ...
                      <FormField control={form.control} name="lastName" render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">Last Name *</FormLabel>
                          <FormControl>
                            <div className="relative group">
                              <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 dark:text-slate-600 group-focus-within:text-emerald-500 transition-colors" />
                              <Input {...field} data-testid="input-last-name" placeholder="Last name" className="pl-11 h-12 bg-slate-50/80 dark:bg-slate-900/40 border-slate-200/80 dark:border-slate-600/60 focus:border-emerald-400 dark:focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 rounded-xl transition-all duration-200 focus:ring-4 focus:ring-emerald-500/10" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField control={form.control} name="email" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <div className="relative group">
                            <Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 dark:text-slate-600 group-focus-within:text-emerald-500 transition-colors" />
                            <Input type="email" {...field} data-testid="input-email" placeholder="your@email.com" className="pl-11 h-12 bg-slate-50/80 dark:bg-slate-900/40 border-slate-200/80 dark:border-slate-600/60 focus:border-emerald-400 dark:focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 rounded-xl transition-all duration-200 focus:ring-4 focus:ring-emerald-500/10" />
                          </div>
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <FormField control={form.control} name="phone" render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">Phone</FormLabel>
                          <FormControl>
                            <div className="relative group">
                              <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 dark:text-slate-600 group-focus-within:text-emerald-500 transition-colors" />
                              <Input {...field} data-testid="input-phone" placeholder="Phone number" className="pl-11 h-12 bg-slate-50/80 dark:bg-slate-900/40 border-slate-200/80 dark:border-slate-600/60 focus:border-emerald-400 dark:focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 rounded-xl transition-all duration-200 focus:ring-4 focus:ring-emerald-500/10" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      ...
                      <FormField control={form.control} name="company" render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">Company *</FormLabel>
                          <FormControl>
                            <div className="relative group">
                              <Building2 className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 dark:text-slate-600 group-focus-within:text-emerald-500 transition-colors" />
                              <Input {...field} data-testid="input-company" placeholder="Company name" className="pl-11 h-12 bg-slate-50/80 dark:bg-slate-900/40 border-slate-200/80 dark:border-slate-600/60 focus:border-emerald-400 dark:focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 rounded-xl transition-all duration-200 focus:ring-4 focus:ring-emerald-500/10" />
                            </div>
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <div className="grid grid-cols-5 gap-3">
                      <div className="col-span-3">
                        <FormField control={form.control} name="city" render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">City</FormLabel>
                            <FormControl>
                              <div className="relative group">
                                <MapPin className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 dark:text-slate-600 group-focus-within:text-emerald-500 transition-colors" />
                                <Input {...field} data-testid="input-city" placeholder="City" className="pl-11 h-12 bg-slate-50/80 dark:bg-slate-900/40 border-slate-200/80 dark:border-slate-600/60 focus:border-emerald-400 dark:focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 rounded-xl transition-all duration-200 focus:ring-4 focus:ring-emerald-500/10" />
                              </div>
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                      ...
                      <div className="col-span-2">
                        <FormField control={form.control} name="province" render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">Province</FormLabel>
                            <FormControl>
                              <div className="relative group">
                                <Map className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 dark:text-slate-600 group-focus-within:text-emerald-500 transition-colors" />
                                <Input {...field} data-testid="input-province" placeholder="e.g. VI" maxLength={2} className="pl-11 h-12 uppercase bg-slate-50/80 dark:bg-slate-900/40 border-slate-200/80 dark:border-slate-600/60 focus:border-emerald-400 dark:focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 rounded-xl transition-all duration-200 focus:ring-4 focus:ring-emerald-500/10" />
                              </div>
                            </FormControl>
                            <FormMessage />
                          </FormItem>
          ...
          <div className="max-w-xl mx-auto" data-testid="card-self-assessment">
            <div className="rounded-3xl bg-white/80 dark:bg-slate-800/70 backdrop-blur-xl border border-white/90 dark:border-slate-700/60 shadow-2xl shadow-emerald-900/5 dark:shadow-black/20 overflow-hidden">
              <div className="px-8 pt-8 pb-4 text-center">
                <div className="w-12 h-12 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-emerald-100 to-teal-100 dark:from-emerald-900/50 dark:to-teal-900/50 flex items-center justify-center">
                  <BookOpen className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
                </div>
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">How do you rate your English?</h2>
                <p className="text-slate-400 dark:text-slate-500 text-sm mt-2">
                  This helps us calibrate the test to your level.
                </p>
              </div>
              <div className="px-6 pb-8 space-y-2.5">
                {Object.entries(CEFR_DESCRIPTIONS).map(([level, { label, desc }]) => {
                  const dotColors: Record<string, string> = {
                    A1: "bg-emerald-500", A2: "bg-teal-500", B1: "bg-cyan-500", B2: "bg-blue-500", C1: "bg-amber-500",
                  };
                  return (
                    <button
                      key={level}
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <ChevronRight className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-emerald-500 dark:group-hover:text-emerald-400 group-hover:translate-x-0.5 transition-all flex-shrink-0" />
                      </div>
                    </button>
                  );
                })}
                {isSubmitting && (
                  <div className="flex flex-col items-center justify-center py-8 gap-3">
                    ...
                    <span className="text-sm text-slate-500 dark:text-slate-400">Preparing the test...</span>
                  </div>
                )}
              </div>
            </div>
                  ...
                  {currentQuestion.passage && (
                    <div className={`rounded-xl p-4 border ${
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        {currentQuestion.skillType === "listening" ? (
                          <>
                            ...
                            <span className="text-xs uppercase tracking-wider text-teal-500 font-medium">Listening Transcript</span>
                          </>
                        ) : (
                      ...
                      <div className={`text-sm whitespace-pre-line leading-relaxed ${
                        currentQuestion.skillType === "listening"
                          ? "text-teal-900 dark:text-teal-200 italic"
          ...
          <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200 dark:border-slate-700 shadow-xl" data-testid="card-writing">
            <CardHeader>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-emerald-700 dark:text-emerald-400">Writing Task</span>
              </div>
              <CardTitle className="text-xl">Writing Task</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="bg-emerald-50 dark:bg-emerald-900/30 rounded-xl p-4 border border-emerald-200 dark:border-emerald-800" data-testid="text-writing-prompt">
                <div className="text-xs uppercase tracking-wider text-emerald-600 mb-2 font-medium">Prompt</div>
              ...
              <div>
                ...
                <div className="flex justify-between mt-2 text-sm text-slate-500">
                  ...
                  <span>{wordCount < 20 ? "Write as much as you can" : wordCount < 50 ? "Good start, keep going" : "Great length"}</span>
                </div>
              </div>
          ...
          <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200 dark:border-slate-700 shadow-xl" data-testid="card-speaking">
            <CardHeader>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-teal-700 dark:text-teal-400">Speaking Task</span>
              </div>
              <CardTitle className="text-xl">Speaking Task</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="bg-teal-50 dark:bg-teal-900/30 rounded-xl p-4 border border-teal-200 dark:border-teal-800" data-testid="text-speaking-prompt">
                <div className="text-xs uppercase tracking-wider text-teal-600 mb-2 font-medium">Prompt</div>
              ...
              <div className="text-center space-y-4">
                {!audioBlob && !isRecording && (
                  <Button
                    ...
                    className="bg-teal-600 hover:bg-teal-700 h-16 px-8 text-lg rounded-2xl"
        ...
        {phase === "results" && results && (
          <div className="space-y-6" data-testid="section-results">
            <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
              <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-8 py-10 text-center text-white">
                <p className="text-emerald-100 text-sm mb-3 uppercase tracking-wider">Your Business English Level</p>
                <div className="text-7xl font-bold mb-2" data-testid="text-final-level">{results.finalLevel}</div>
                <p className="text-emerald-200">CEFR Level</p>
              </div>
              <CardContent className="p-6">
                  ...
                  <div className="mt-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        <div key={label} className="p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
                  ...
                  <div className="mt-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        <div key={label} className="p-3 bg-teal-50 dark:bg-teal-900/20 rounded-lg">
                          <div className="text-xs text-teal-600 mb-1">{label}</div>
                          <div className="flex items-center gap-2">
                            <Progress value={val} className="flex-1 h-2" />
                            <span className="text-sm font-bold text-slate-700 dark:text-slate-300">{val}</span>
                          </div>
                ...
                <div className="mt-8 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800 text-center">
                  <CheckCircle className="w-6 h-6 text-emerald-600 mx-auto mb-2" />
                  <p className="text-emerald-800 dark:text-emerald-300 font-medium">Assessment Complete</p>
                  <p className="text-sm text-emerald-600 dark:text-emerald-400 mt-1">
                    Your results have been sent to your company. Our team will follow up with tailored training recommendations.
                  </p>
                </div>
              </CardContent>
            </Card>
