...
const CEFR_DESCRIPTIONS: Record<string, { label: string; desc: string }> = {
  A1: { label: "A1 - Principiante", desc: "Puedo comprender frases muy sencillas y presentarme en situaciones simples." },
  A2: { label: "A2 - Elemental", desc: "Puedo manejar conversaciones cotidianas simples y escribir mensajes cortos." },
  B1: { label: "B1 - Intermedio", desc: "Puedo hablar sobre temas familiares, comprender los puntos principales de textos claros y escribir textos sencillos y coherentes." },
  B2: { label: "B2 - Intermedio Superior", desc: "Puedo participar en discusiones detalladas, comprender textos complejos y redactar ensayos claros." },
  C1: { label: "C1 - Avanzado", desc: "Puedo expresarme con fluidez y espontaneidad, comprender textos exigentes y producir escritos bien estructurados y detallados." },
};

const SKILL_LABELS: Record<string, string> = {
  grammar: "Gramática",
  vocabulary: "Vocabulario",
  use_of_english: "Uso del Idioma",
...
export default function SpanishTestPage() {
  useSEO({
    title: "Test di Spagnolo Gratuito Online | Livello CEFR | SkillCraft-Interlingua",
    description: "Test di livello di spagnolo gratuito online. Valutazione adattiva CAT/IRT per scoprire il tuo livello CEFR (A1-C2) in pochi minuti. Risultati immediati.",
    canonical: "/test-di-livello/spagnolo",
  });
  ...
  const handleSelfAssessment = async (level: string) => {
    ...
    try {
      const res = await apiRequest("POST", "/api/spanish-test/start", {
        ...registrationData,
  ...
  const handleAnswer = async () => {
    ...
    try {
      const res = await apiRequest("POST", "/api/spanish-test/answer", {
        sessionId,
  ...
  const handleWritingSubmit = async () => {
    ...
    setIsSubmitting(true);
    try {
      const res = await apiRequest("POST", "/api/spanish-test/submit-writing", {
        sessionId,
      ...
      if (data.success) {
        ...
        const skipRes = await apiRequest("POST", "/api/spanish-test/complete-without-speaking", { sessionId });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50/50 to-violet-50 dark:from-slate-900 dark:via-blue-950 dark:to-indigo-950 relative overflow-hidden notranslate" translate="no">
      <Breadcrumb items={[{ label: "Test di Livello", href: "/test-di-livello" }, { label: "Test di Spagnolo" }]} schemaOnly />
        ...
        {(phase === "registration" || phase === "audio-check" || phase === "self-assessment") && (
          <div className="text-center mb-10">
            ...
            <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-slate-900 via-blue-900 to-indigo-900 dark:from-white dark:via-blue-100 dark:to-indigo-200 bg-clip-text text-transparent leading-normal pb-2" data-testid="text-page-title">
              Descubre Tu Nivel<br className="hidden md:block" /> de Español
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-3 text-base max-w-lg mx-auto">
              Un test gratuito e inteligente que se adapta a tus respuestas para evaluar con precisión tus competencias lingüísticas.
            </p>
          </div>
        )}
        {phase !== "registration" && phase !== "audio-check" && phase !== "self-assessment" && (
            ...
            {phase === "mc-questions" && (
              <div className="mt-3">
                ...
                <div className="flex justify-between mt-1 text-xs text-slate-500">
                  {["Gramát", "Vocab", "Uso", "Lectura", "Audio"].map((label, idx) => (
            ...
            <div className="rounded-3xl bg-white/80 dark:bg-slate-800/70 backdrop-blur-xl border border-white/90 dark:border-slate-700/60 shadow-2xl shadow-blue-900/5 dark:shadow-black/20 overflow-hidden">
              <div className="px-8 pt-8 pb-4 text-center">
                ...
                <p className="text-amber-600 dark:text-amber-400 text-xs mt-4 max-w-sm mx-auto">
                  Para una evaluación más precisa, te recomendamos hacer el test con el audio activado.
                </p>
                <button
                  onClick={() => {
                    ...
                    setPhase("self-assessment");
                    toast({ title: "Audio desactivado", description: "Las preguntas de comprensión auditiva se mostrarán como texto.", variant: "destructive" });
                  }}
                ...
                >
                  <VolumeX className="w-4 h-4" /> No tengo audio — saltar
                </button>
              </div>
          ...
          <div className="max-w-xl mx-auto" data-testid="card-self-assessment">
            <div className="rounded-3xl bg-white/80 dark:bg-slate-800/70 backdrop-blur-xl border border-white/90 dark:border-slate-700/60 shadow-2xl shadow-blue-900/5 dark:shadow-black/20 overflow-hidden">
              <div className="px-8 pt-8 pb-4 text-center">
                ...
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">Come valuti il tuo spagnolo?</h2>
                <p className="text-slate-400 dark:text-slate-500 text-sm mt-2">
          ...
          <div className="space-y-6" data-testid="section-results">
            <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-10 text-center text-white">
                <p className="text-blue-100 text-sm mb-3 uppercase tracking-wider">Your Spanish Level</p>
